...
> 7. **Personal capacity derivation (mandatory):** Each week plan must declare its personal-capacity model from that week's actual schedule — NOT from a fixed template shorthand. Required declarations: (a) weekday evening availability (days, start time = 19:30, 2h/evening, baseline total = 5 × 2h = 10h/week); (b) weekend daytime availability (Sat daytime: substantial / partial / none; Sun daytime: substantial / partial / none); (c) protected rest windows — Saturday evening and Sunday evening are OFF by default and must not be allocated; (d) whether personal goals depend on weekend daytime extension or fit within weekday evening baseline alone. Plans must NOT use `~6–8h/week personal` as a shorthand unless that week has genuinely constrained availability. Any personal capacity wording copied from a prior week must be revalidated before committing. See `CAPACITY_ENGINE.md` R10.
